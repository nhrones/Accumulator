...
Changes to the original encoder are simply:
  ...
  Replaced the byteParts: Uint8Array[], with an Accumulator instance;

  in several places, I renamed the parameter `object` to the name `item` for
  readability. I found the name `object` a bit confusing in a few places.
...
function encodeSlice(item: ValueType, accumulator: Accumulator) {
   ...
   if (typeof item === "string") {
      ...
      if (len < FIVE_BITS) { // fixstr
         accumulator.appendByte(0xa0 | len)
      } else if (len < EIGHT_BITS) { // str 8
         accumulator.appendByte(0xd9)
         accumulator.appendByte(len)
